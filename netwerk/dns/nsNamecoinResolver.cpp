...
nsresult nsElectrumXPooledConnection::EnsureConnected() {
  ...
  if (!ParseUrl(mServerUrl, host, port, path)) {
    NMC_ERR("EnsureConnected: bad URL: %s", mServerUrl.get());
    mClosed = true;
    return NS_ERROR_MALFORMED_URI;
  }

  NMC_LOG("EnsureConnected: TCP connect to %s:%d%s",
  ...
  if (PR_StringToNetAddr(host.get(), &addr) != PR_SUCCESS) {
    ...
    if (PR_GetHostByName(host.get(), buf, sizeof(buf), &he) != PR_SUCCESS) {
      NMC_ERR("EnsureConnected: DNS lookup failed for %s", host.get());
      mClosed = true;
  ...
  if (!mFd) {
    NMC_ERR("EnsureConnected: PR_OpenTCPSocket failed");
    mClosed = true;
    return NS_ERROR_FAILURE;
  }
  ...
  if (PR_Connect(mFd, &addr, timeout) != PR_SUCCESS) {
    ...
    NMC_ERR("EnsureConnected: PR_Connect failed: %d", (int)err);
    PR_Close(mFd);
    mFd = nullptr;
    mClosed = true;
  ...
  NMC_LOG("EnsureConnected: TCP connected, doing WS handshake");
  ...
  if (sent != (PRInt32)req.Length()) {
    NMC_ERR("EnsureConnected: send failed, sent=%d expected=%u",
  ...
  while (true) {
    ...
    if (n <= 0) {
      NMC_ERR("EnsureConnected: recv failed during handshake");
      Close();
      return NS_ERROR_FAILURE;
    }
    ...
    if (response.Length() > 4096) {
      NMC_ERR("EnsureConnected: response too large");
      Close();
      return NS_ERROR_FAILURE;
    }
  }
  ...
  if (!StringBeginsWith(response, "HTTP/1.1 101"_ns)) {
    NMC_ERR("EnsureConnected: WS upgrade rejected: %s",
            response.get());
    Close();
    return NS_ERROR_FAILURE;
  }

  NMC_LOG("OnStart: WebSocket connected to %s", mServerUrl.get());
  return NS_OK;
}
...
nsresult nsElectrumXPooledConnection::RecvFrame(nsCString& aData) {
  ...
  uint8_t header[2];
  if (!recvN(header, 2)) {
    NMC_ERR("RecvFrame: failed to read header");
    return NS_ERROR_FAILURE;
  }
  ...
  if (payloadLen > 1024 * 1024) {  // 1 MB safety limit
    NMC_ERR("RecvFrame: frame too large (%llu bytes)",
  ...
  if (payloadLen > 0 && !recvN(payload.Elements(), (uint32_t)payloadLen)) {
    NMC_ERR("RecvFrame: failed to read payload");
    return NS_ERROR_FAILURE;
  }
...
nsresult nsElectrumXPooledConnection::SendRequest(int32_t aReqId,
  ...
  NMC_LOG("SendRequest: id=%d request=%s",
          aReqId, aRequest.get());
  ...
  while (true) {
    if (PR_IntervalNow() > deadline) {
      NMC_ERR("SendRequest: timeout waiting for id=%d", aReqId);
      Close();
    ...
    NMC_LOG("OnMessageAvailable: id=%d frame_len=%u",
...
nsresult nsNamecoinResolver::Init() {
  ...
  NMC_LOG("Init(): enabled=%s", mEnabled ? "true" : "false");
  if (!mEnabled) {
  ...
  while (tokenizer.hasMoreTokens()) {
    ...
    if (!server.IsEmpty()) {
      ...
      NMC_LOG("Init(): server: %s", server.get());
    }
  }
  ...
  NMC_LOG("Init(): initialized OK, %u servers, timeout=%ums",
          (unsigned)mServers.Length(), mConnectionTimeoutMs);
  return NS_OK;
}
...
nsresult nsNamecoinResolver::ResolveInternal(const nsACString& aHostname,
  ...
  MOZ_ASSERT(!NS_IsMainThread());
  NMC_LOG("ResolveInternal: START %s",
          nsPromiseFlatCString(aHostname).get());
  ...
  NMC_LOG("ResolveInternal: scripthash=%s", scripthash.get());
  ...
  NMC_LOG("ResolveInternal: history done, bestTx=%s height=%u",
          bestTxHash.get(), bestHeight);
  ...
  NMC_LOG("ResolveInternal: tx fetched len=%u", (unsigned)txJson.Length());
  ...
  NMC_LOG("ResolveInternal: nameValue='%s'", nameValue.get());

  // Step 9: Parse value JSON
  rv = ParseNameValue(nameValue, aHostname, aResult.nameValue);
  NS_ENSURE_SUCCESS(rv, rv);
  NMC_LOG("ResolveInternal: parsed ip=%s ip6=%s",
  ...
  NMC_LOG("ResolveInternal: SUCCESS %s ip=%s ip6=%s ttl=%us",
          nsPromiseFlatCString(aHostname).get(),
          aResult.nameValue.ip.get(), aResult.nameValue.ip6.get(),
          aResult.ttlSeconds);
  return NS_OK;
