...
nsresult nsHostResolver::NameLookup(nsHostRecord* rec,
  ...
  // that path — the DNS resolver thread pool will handle it separately.
  {
    ...
    if (!NS_IsMainThread() && nsNamecoinResolver::IsNamecoinHost(host) &&
      ...
      if (sNmcResolver) {
        LOG(("NameLookup [namecoin]: .bit domain detected: %s",
             host.get()));
        ...
        // MutexAutoUnlock re-acquires mLock on destruction (end of block).
        {
          ...
          LOG(("NameLookup [namecoin]: resolving %s via ElectrumX",
               hostCopy.get()));
        ...
        if (NS_SUCCEEDED(nmcRv) && nmcResult.resolved &&
          ...
          // Populate the host record and call CompleteLookupLocked
          // directly, bypassing normal DNS entirely.
          //
          // We hold mLock here. CompleteLookupLocked requires:
          //   1. mLock is held
          //   2. addrRec->mResolving > 0
          RefPtr<AddrHostRecord> addrRec = do_QueryObject(rec);
          if (addrRec) {
            nsTArray<NetAddr> addrs(nmcResult.addresses.Clone());
            RefPtr<AddrInfo> ai =
          ...
          LOG(("NameLookup [namecoin]: resolve failed for %s: rv=%08x "
               "resolved=%s addrs=%zu error=%s",
               hostCopy.get(),
               (unsigned)nmcRv,
        ...
        // Fall through to normal DNS (resolution failed).
      }
    }
  }
